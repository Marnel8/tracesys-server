...
const sanitizeRedirectPath = (raw?: string): string | undefined => {
  ...
  if (
    raw.startsWith("/dashboard/instructor") ||
...
export const initiateGoogleOAuth = async (req: Request, res: Response) => {
  try {
    ...
    res.redirect(
      `${clientUrl}/auth/error?error=oauth_initiate_failed&message=${encodeURIComponent(
...
export const handleGoogleOAuthCallback = async (
  ...
  try {
    ...
    if (error) {
      ...
      return res.redirect(
        `${clientUrl}/auth/error?error=oauth_denied&message=${encodeURIComponent(
    ...
    if (existingUser) {
      ...
      if (!existingUser.isActive) {
        return res.redirect(
          `${clientUrl}/auth/error?error=account_deactivated&message=${encodeURIComponent(
      ...
      if (decodedState.invitationToken) {
        try {
          await markInvitationAsUsed(decodedState.invitationToken);
        } catch (error) {
      ...
      if (!decodedState.invitationToken) {
        return res.redirect(
          `${clientUrl}/auth/error?error=invitation_required&message=${encodeURIComponent(
      ...
      const invitation = await validateInvitationToken(
        decodedState.invitationToken
      );

      // Check if email matches invitation
      if (invitation.email !== googleUser.email) {
        return res.redirect(
          `${clientUrl}/auth/error?error=email_mismatch&message=${encodeURIComponent(
            "Google account email does not match invitation email"
          )}`
        );
      }
    ...
    res.redirect(
      `${clientUrl}/auth/error?error=${errorCode}&message=${encodeURIComponent(
        errorMessage
      )}`
    );
  }
