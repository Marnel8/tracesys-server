...
export const createInvitationController = async (req: Request, res: Response) => {
	const { email, role, departmentId, sectionId, program, expiresInDays }: CreateInvitationRequest = req.body;
	...
	if (departmentId) invitationUrl.searchParams.set("departmentId", departmentId);
...
export const createBulkInvitationsController = async (req: Request, res: Response) => {
	const { emails, role, departmentId, sectionId, program, expiresInDays }: BulkInvitationRequest = req.body;
	const instructorId = (req as any).user?.id;
