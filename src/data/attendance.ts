...
const calculateTimeBoundaries = (agency: Agency | null) => {
  ...
  if (agency) {
    if (opening !== null && closing !== null) {
      ...
      if (!isValid) {
        console.warn(
          `[Time validation] Invalid time range: opening (${agency.openingTime}) should be before closing (${agency.closingTime})`
        );
      }
    }
    ...
    if (lunchStart !== null && lunchEnd !== null) {
      ...
      if (!isValid) {
        console.warn(
          `[Time validation] Invalid lunch time range: lunchStart (${agency.lunchStartTime}) should be before lunchEnd (${agency.lunchEndTime})`
        );
      }
      ...
      if (opening !== null && closing !== null) {
        ...
        if (!lunchStartValid || !lunchEndValid) {
          console.warn(
            `[Time validation] Lunch times (${agency.lunchStartTime} - ${agency.lunchEndTime}) should be within workday (${agency.openingTime} - ${agency.closingTime})`
          );
        }
      }
    }
...
export const clockOutData = async (params: ClockOutParams) => {
  ...
  if (sessionType === "morning") {
    ...
    if (overtimeHours > 2.0) {
      throw new Error(
        "Overtime session cannot exceed 2 hours. Maximum overtime limit reached."
      );
    }
  ...
  const undertimeHours = calculateUndertimeHours(
    params.agency ?? null,
    totalHours, // Use totalHours (without overtime) for undertime calculation
    record.morningTimeIn || updateData.morningTimeIn || null,
    updateData.morningTimeOut || record.morningTimeOut || null,
    record.afternoonTimeIn || updateData.afternoonTimeIn || null,
    updateData.afternoonTimeOut || record.afternoonTimeOut || null
...
export const approveAttendanceData = async (
  id: string,
  approverId: string,
  notes?: string | null
) => {
...
export const rejectAttendanceData = async (
  id: string,
  approverId: string,
  reason?: string | null
) => {
