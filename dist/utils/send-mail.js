...
const sendMail = async (options) => {
    ...
    console.log(`${logPrefix}   - SMTP_HOST: ${smtpHost ? "✓ Set" : "✗ Missing"}`);
    console.log(`${logPrefix}   - SMTP_MAIL: ${smtpMail ? "✓ Set" : "✗ Missing"}`);
    console.log(`${logPrefix}   - SMTP_PASSWORD: ${smtpPassword ? "✓ Set (" + smtpPassword.length + " chars)" : "✗ Missing"}`);
    console.log(`${logPrefix}   - MAIL (from): ${fromEmail || "Not set, using SMTP_MAIL"}`);
    console.log(`${logPrefix}   - SMTP_PORT: ${process.env.SMTP_PORT || "587 (default)"}`);
    console.log(`${logPrefix}   - SMTP_SERVICE: ${process.env.SMTP_SERVICE || "Not set"}`);
    ...
    if (!smtpHost || !smtpMail || !smtpPassword) {
        ...
        const errorMsg = `Missing required email configuration: ${missing.join(", ")}. Please check your environment variables.`;
    ...
    console.log(`${logPrefix} [STEP 1] SMTP details - Host: ${smtpHost}, Port: ${process.env.SMTP_PORT || "587"}, User: ${smtpMail}`);
    ...
    if (spacesRemoved > 0) {
        console.log(`${logPrefix} [STEP 2] Removed ${spacesRemoved} space(s) from password`);
    }
    console.log(`${logPrefix} [STEP 2] ✓ Password processed (${cleanedPassword.length} chars)`);
    ...
    console.log(`${logPrefix} [STEP 3] Port: ${smtpPort}, Secure (SSL): ${isSecure}, Connection type: ${isSecure ? "SSL/TLS (port 465)" : "STARTTLS (port 587)"}`);
    ...
    const emailDomain = fromEmail?.split("@")[1] || smtpMail?.split("@")[1] || "tracesys";
    const transporterName = process.env.SMTP_NAME || emailDomain;
    console.log(`${logPrefix} [STEP 4] Transporter name (SMTP identification): ${transporterName}`);
    ...
    console.log(`${logPrefix}   - RequireTLS: ${transporterConfig.requireTLS || false}`);
    console.log(`${logPrefix}   - TLS rejectUnauthorized: ${transporterConfig.tls?.rejectUnauthorized || false}`);
    console.log(`${logPrefix}   - Timeouts: ${transporterConfig.connectionTimeout}ms`);
    ...
    if (shouldVerify) {
        try {
            console.log(`${logPrefix} [STEP 5] Attempting to verify connection to ${smtpHost}:${smtpPort}...`);
            const verifyStartTime = Date.now();
            await Promise.race([
                transporter.verify(),
                new Promise((_, reject) => setTimeout(() => reject(new Error("SMTP verification timeout after 10 seconds")), 10000)),
            ]);
            const verifyDuration = Date.now() - verifyStartTime;
            console.log(`${logPrefix} [STEP 5] ✓ SMTP connection verified successfully (took ${verifyDuration}ms)`);
            console.log(`${logPrefix} [STEP 5] Server is ready to accept messages`);
        }
        catch (error) {
            const errorMsg = `SMTP verification failed: ${error instanceof Error ? error.message : "Unknown error"}`;
            console.error(`${logPrefix} [STEP 5] ✗ VERIFICATION FAILED: ${errorMsg}`);
            if (error instanceof Error) {
                console.error(`${logPrefix} [STEP 5] Error details:`, error);
                if (error.code) {
                    console.error(`${logPrefix} [STEP 5] Error code: ${error.code}`);
                }
            }
            // Don't throw - continue anyway as verification can fail in some production environments
            // but actual sending might still work
            console.warn(`${logPrefix} [STEP 5] ⚠ Continuing despite verification failure (will attempt to send anyway)...`);
        }
    }
    else {
        console.log(`${logPrefix} [STEP 5] ⚠ SMTP verification skipped (SMTP_VERIFY=false)`);
    }
    console.log(`${logPrefix} [STEP 6] Preparing email content...`);
    const { email, subject, template, data, html: directHtml, text: directText, } = options;
    let html;
    let textVersion;
    // If direct html/text provided, use those (no template engine)
    if (directHtml || directText) {
        console.log(`${logPrefix} [STEP 6] Using direct content (no template engine)`);
        html = directHtml || "";
        textVersion =
            directText || (directHtml ? extractTextFromHTML(directHtml) : "");
        console.log(`${logPrefix} [STEP 6] HTML length: ${html.length} chars`);
        console.log(`${logPrefix} [STEP 6] Text length: ${textVersion.length} chars`);
    }
    else if (template) {
        ...
        console.log(`${logPrefix} [STEP 6] Template data keys: ${data ? Object.keys(data).join(", ") : "none"}`);
        ...
        candidateMailDirs.forEach((dir, index) => {
            ...
            console.log(`${logPrefix} [STEP 6]   ${index + 1}. ${dir} ${exists ? "✓" : "✗"}`);
        });
        ...
        console.log(`${logPrefix} [STEP 6] Template exists: ${fs_1.default.existsSync(templatePath)}`);
        ...
        console.log(`${logPrefix} [STEP 6] ✓ Template found, rendering with EJS...`);
        try {
            const renderStartTime = Date.now();
            html = await ejs_1.default.renderFile(templatePath, data || {});
            const renderDuration = Date.now() - renderStartTime;
            console.log(`${logPrefix} [STEP 6] ✓ Template rendered successfully (took ${renderDuration}ms)`);
            console.log(`${logPrefix} [STEP 6] Rendered HTML length: ${html.length} characters`);
            ...
            console.log(`${logPrefix} [STEP 6] HTML structure check - DOCTYPE: ${hasDoctype}, HTML tag: ${hasHtmlTag}, Body tag: ${hasBodyTag}`);
            // Log first 200 chars to verify content
            console.log(`${logPrefix} [STEP 6] HTML preview (first 200 chars): ${html.substring(0, 200)}...`);
        }
        catch (error) {
            const errorMsg = `Failed to render email template: ${error instanceof Error ? error.message : "Unknown error"}`;
    ...
    if (!fromEmail) {
        console.error(`${logPrefix} [STEP 7] ✗ From email address is not configured`);
    ...
    console.log(`${logPrefix} [STEP 7] Base headers:`, Object.keys(headers).join(", "));
    ...
    console.log(`${logPrefix} [STEP 7] Marketing headers enabled: ${isMarketingEmail}`);
    if (isMarketingEmail) {
        ...
        if (clientUrl) {
            ...
            console.log(`${logPrefix} [STEP 7] No CLIENT_URL/FRONTEND_URL found, skipping List-Unsubscribe`);
        }
    }
    // DO NOT add "Precedence: bulk" - it triggers spam filters for transactional emails
    console.log(`${logPrefix} [STEP 7] ✓ Headers prepared (${Object.keys(headers).length} headers)`);
    console.log(`${logPrefix} [STEP 8] Building email message...`);
    // Text version already prepared in STEP 6
    if (!textVersion) {
        console.log(`${logPrefix} [STEP 8] Extracting plain text version from HTML...`);
        const textStartTime = Date.now();
        textVersion = extractTextFromHTML(html);
        const textDuration = Date.now() - textStartTime;
        console.log(`${logPrefix} [STEP 8] ✓ Text version extracted (${textVersion.length} chars, took ${textDuration}ms)`);
    }
    else {
        console.log(`${logPrefix} [STEP 8] Using provided text version (${textVersion.length} chars)`);
    }
    ...
    if (attachmentsCount > 0) {
        options.attachments?.forEach((att, index) => {
            console.log(`${logPrefix} [STEP 8]   ${index + 1}. ${att.filename} (${att.path})`);
        });
    }
    ...
    if (!mailOptions.from) {
        console.warn(`${logPrefix} [STEP 8] ⚠ 'from' field was missing, setting fallback`);
        mailOptions.from = fromEmail || smtpMail;
    }
    console.log(`${logPrefix} [STEP 8] From field: ${mailOptions.from} (REQUIRED for custom SMTP)`);
    ...
    console.log(`${logPrefix}   - HTML size: ${mailOptions.html?.length || 0} chars`);
    console.log(`${logPrefix}   - Text size: ${mailOptions.text?.length || 0} chars`);
    console.log(`${logPrefix}   - Attachments: ${attachmentsCount}`);
    console.log(`${logPrefix}   - Headers: ${Object.keys(mailOptions.headers).length}`);
    ...
    try {
        ...
        const response = info.response || "unknown";
        ...
        console.log(`${logPrefix} [STEP 9] SMTP Response: ${JSON.stringify(response)}`);
        console.log(`${logPrefix} [STEP 9] Accepted recipients: ${accepted.length > 0 ? accepted.join(", ") : "none"}`);
        console.log(`${logPrefix} [STEP 9] Rejected recipients: ${rejected.length > 0 ? rejected.join(", ") : "none"}`);
        if (info.pending) {
            console.log(`${logPrefix} [STEP 9] Pending recipients: ${info.pending.join(", ")}`);
        }
        ...
        console.log(`${logPrefix} [STEP 9]   - If recipient doesn't receive it, check:`);
        console.log(`${logPrefix} [STEP 9]     1. Sender's Gmail Sent folder (should appear there)`);
        ...
        console.log(`${logPrefix} [STEP 9]     4. Check Gmail account security settings`);
        console.log(`${logPrefix} [STEP 9]     5. Message ID for tracking: ${messageId}`);
        console.log(`${logPrefix} ========================================`);
    }
    catch (error) {
        ...
        if (error instanceof Error) {
            ...
            if (error.code) {
                console.error(`${logPrefix} [STEP 9] Error code: ${error.code}`);
            }
            if (error.command) {
                console.error(`${logPrefix} [STEP 9] Failed SMTP command: ${error.command}`);
            }
            if (error.response) {
                console.error(`${logPrefix} [STEP 9] SMTP response: ${error.response}`);
            }
            if (error.responseCode) {
                console.error(`${logPrefix} [STEP 9] SMTP response code: ${error.responseCode}`);
            }
            if (error.errno) {
                console.error(`${logPrefix} [STEP 9] System error number: ${error.errno}`);
            }
            if (error.syscall) {
                console.error(`${logPrefix} [STEP 9] System call: ${error.syscall}`);
            }
            if (error.hostname) {
                console.error(`${logPrefix} [STEP 9] Hostname: ${error.hostname}`);
            }
        }
        else {
            console.error(`${logPrefix} [STEP 9] Unknown error type:`, typeof error, error);
        }
